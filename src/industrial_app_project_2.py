...
def findCountrsInImage(frameImg, showImage):
    contours, hierarchy = cv2.findContours(
        frameImg, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    hull = []
    ...
    for i in range(0, len(contours)):
        ...
        if (area_min < area < area_max):
            ...
            x_coor = int(center[0])
            y_coor = int(center[1])
            cv2.putText(showImage, f'*', (x_coor - 15, y_coor + 5),
            ...
            cv2.rectangle(showImage, (x - 20, y),
                          (x + w, y + h), (0, 255, 0), 2)
...
def main():
    ...
    while True:
        ...
        _, thresholdImg = cv2.threshold(
            morphClose, tmin, tmax, cv2.THRESH_TOZERO)
